...
"""Utility functions for StegX: File I/O, Compression, Metadata Handling, Logging Setup."""
...
def setup_logging(level=logging.INFO):
    ...
    # Corrected format string without unnecessary escapes
...
def create_payload(file_path: str, compress: bool = True) -> bytes:
    """Reads a file, optionally compresses it, and creates a payload with metadata.

    Payload format: [4-byte little-endian metadata length] + [JSON metadata] + [file data]

    Args:
        file_path: Path to the file to include in the payload.
        compress: Whether to attempt compression (default: True).

    Returns:
        The combined payload as bytes.

    Raises:
        FileNotFoundError: If the input file does not exist.
        IOError: If there is an error reading the file.
        Exception: For other errors like JSON serialization.
    """
    ...
    try:
        ...
        metadata = {
            ...
            META_FILENAME: filename,
            META_ORIG_SIZE: original_size, # Store original size even if compressed
            META_COMPRESSED: is_compressed
        }

        metadata_bytes = json.dumps(metadata, ensure_ascii=False).encode("utf-8")
        # Corrected byteorder string literal
        ...
        logging.exception(f"Error creating payload for {file_path}:")
        raise

def parse_payload(payload: bytes) -> Tuple[str, bytes]:
    """Parses a payload to extract metadata and file data.

    Payload format: [4-byte little-endian metadata length] + [JSON metadata] + [file data]

    Args:
        payload: The combined payload bytes.

    Returns:
        A tuple containing (original_filename, original_file_data).

    Raises:
        ValueError: If the payload is corrupt or metadata is invalid.
        zlib.error: If decompression fails.
        Exception: For other errors like JSON deserialization.
    """
    ...
    # Corrected structure and byteorder string literal
    try:
        ...
        # Validate metadata (basic checks)
        ...
        if metadata[META_VERSION] > CURRENT_META_VERSION:
            logging.warning(f"Payload metadata version ({metadata[META_VERSION]}) is newer than supported ({CURRENT_META_VERSION}). Attempting to parse anyway.")
        ...
        if is_compressed:
            ...
            try:
                ...
                if len(decompressed_data) != original_size:
                    # Corrected f-string
                    logging.warning(f"Decompressed size ({len(decompressed_data)}) does not match original size ({original_size}) stored in metadata for '{filename}'. Using decompressed data.")
            ...
            if len(file_data) != original_size:
                 # Corrected f-string
                 logging.warning(f"Stored data size ({len(file_data)}) does not match original size ({original_size}) stored in metadata for ", {filename}," Using stored data.")
        ...
        logging.exception("Unexpected error during payload parsing:")
        raise

def save_extracted_file(filename: str, data: bytes, output_dir: str):
    """Saves the extracted data to a file in the specified directory."""
    if not os.path.isdir(output_dir):
        logging.error(f"Output directory does not exist: {output_dir}")
        raise FileNotFoundError(f"Output directory not found: {output_dir}")

    # Basic sanitization of filename (prevent path traversal)
    safe_filename = os.path.basename(filename)
    if not safe_filename or safe_filename in (".", ".."):
        safe_filename = "extracted_file.dat" # Default fallback name
        # Corrected f-string
    ...
    try:
        ...
        logging.exception(f"Unexpected error saving file {output_path}:")
        raise
