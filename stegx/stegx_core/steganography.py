...
def generate_pixel_positions(width: int, height: int, channels: int, seed: int = 42) -> List[Tuple[int, int, int]]:
    random.seed(seed)
    positions = []
...
def embed_data(cover_image_path: str, data_to_hide: bytes, output_image_path: str):
    ...
    try:
        ...
        positions = generate_pixel_positions(width, height, channels)
